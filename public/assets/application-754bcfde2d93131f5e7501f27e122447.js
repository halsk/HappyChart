...
var doit =  function () {
    ...
    var happy = (-8 + attr.edu_choice*1 + attr.env_choice*1 +attr.commu_choice*1
			      +attr.work_choice*1 + attr.safety_choice*1 + attr.nature_choice*1 
			      +attr.income_choice*1 + attr.freetime_choice*1) *100/  32 ;
    var shappy = parseInt(happy*10)/10;
    var cval = attr.edu_choice +attr.commu_choice  + attr.env_choice +
	attr.safety_choice +  attr.nature_choice +
	attr.work_choice + attr.income_choice +  attr.freetime_choice;

    // put values into chart
    //    console.log("cval="+cval);
    $.cookie("enq",cval, { expires: 30});
    //    console.log($.cookie("enq"));

    var sex = attr.sex;
    //    console.log("sex="+sex);
    $.cookie("sex",sex , { expires: 30});

    var fam = attr.fam_choice;
    //    console.log("fam="+fam);
    $.cookie("fam",fam , { expires: 30});

    var job = attr.job_choice;
    //    console.log("job="+job);
    $.cookie("job",job , { expires: 30});

    var alog = attr.location;
    $.cookie("loc",alog, { expires: 30});

    $('#container0').highcharts({
	    chart: { polar: true, type: 'area' },
	    title: {      text: 'わたしのハッピー度 ('+shappy+'%)',    },
	    pane: {size: '80%'  },
	    xAxis: {
	    ...
	    legend: {
		 ...
		 series: [ {
		            name: 'わたし',
		            data: [attr.edu_choice*100, attr.commu_choice*100, attr.env_choice*100, 
			         attr.safety_choice*100, attr.nature_choice*100, attr.work_choice * 100
			      ,attr.income_choice*100, attr.freetime_choice*100 ],
		            pointPlacement: 'on',
		        }]
    ...
    var postcode = [[10,20,13],[21,25,14],[26,29,12],[30,31,08],[32,32,09],
                    [33,36,11],[37,37,10],[38,39,20],[40,40,19],[41,43,22],
                    [44,49,23],[50,50,21],[51,51,24],[52,52,25],[53,59,27],
                    [60,62,26],[63,63,29],[64,64,30],[65,67,28],[68,68,31],
                    [69,69,32],[70,71,33],[72,73,34],[74,75,35],[76,76,37],
                    [77,77,36],[78,78,39],[79,79,38],[80,83,40],[84,84,41],
                    [85,85,42],[86,86,43],[87,87,44],[88,88,45],[89,89,46],
                    [90,90,47],[91,91,18],[92,92,17],[93,93,16],[94,95,15],
                    [96,97,07],[98,98,04],[99,99,06],[01,01,05],[02,02,03],
                    [03,03,02],[04,09,01],[00,00,01]];
...
//				     console.log("Now:Pref Count = "+results.length);                             
					 ...
					 pd.save(null,{
					     success: function(user){ console.log("pref success save");},
					 ...
					 pd.increment("myct",1);
					 var count = pd.get("myct");
					 console.log("Count is :"+count);
					 arp = divArray(arp, (count-1)/count);
					 ar = divArray(ar,count);
					 ar = addArray(ar,arp);
					 for(i=0; i< arp.length; i++){
					     pd.set(""+i,ar[i]);
					 ...
					 pd.save(null,{
					     success: function(user){ console.log("pref success inc save");},
					     error: function(error){}
					 });
...
(function(root) {
  ...
  NCMB._request = function(route, className, objectId, method, dataObject) {
    ...
    //Process route of file/user/push/class
...
(function(root) {
  ...
  /**
   * プロミスを実施。プロミスは非同期の処理を行った後に予約したコールバックを呼び出す。
   *
   * <p>利用例:<pre>
...
(function(root) {
   ...
   * @class
   *
   * <p>エラーコールバックに渡されたオブジェクトのために利用する。</p>
   */
...
(function() {
   ...
   * @class
   *
   * <p>NCMB.EventsはBackboneのEventsモジュールの分枝。</p>
   *
...
(function(root) {
   ...
   * @class
   *
   * <p>Backboneモデルインターフェイスを実行するNCMBデータの基本クラス。</p>
   */
...
(function(root) {
   ...
   *
   * <p>
   * 多対多のリレーションのオブジェクトをアクセスするため利用。 
   * NCMB.Relationのインスタンスは特定の親オブジェクトとキーと関連させる。
   * </p>
   */
...
(function(root) {
  ...
  /**
   * @class
   * NCMB.OpはNCMB.Objectのフィールドに適応できる操作単位である。例えば、<code>object.set("foo", "bar")</code>
   * はNCMB.Op.Setの例である。 <code>object.unset("foo")</code>はNCMB.Op.Unsetの一つの例である。
   * このような操作はNCMB.Objectに保存され、<code>object.save()</code> の一部としてサーバーに送信する。
   * NCMB.Opのインスタンスは不変性である。直接NCMB.
   * Opのサブクラスを作成しない方がお勧めする。
   */
  NCMB.Op = function() {
...
(function(root) {
  ...
  /**
   * NCMBのファイルを操作するクラスである。
   * @name NCMB.File
   * @namespace
   */


  NCMB.File = NCMB.Object.extend("File", 
  /** @lends NCMB.File.prototype */
  {
  /**
   *
   * NCMB.FileはNCMBに保存するためのファイルのロカール表現。　
...
(function(root) {
   ...
   * @class
   *
   * <p>ユーザーのアクセスを制限させるため、ACLアクセスコントロールリストはすべて<code>NCMB.Object</code>に追加可能である。</p>
   */
  ...
  /**
   * 渡されたユーザーIDが明確にオブジェクトを読み込み権限を持っているかどうか。 
...
(function(root) {
   ...
   * @class
   * NCMB.RoleはNCMBクラウドのロールに対応するロカール表現である。
   */
  NCMB.Role = NCMB.Object.extend("role", /** @lends NCMB.Role.prototype */ {
     ...
     */
    getUsers: function() {
      return this.relation("belongUser");
    },
     ...
     */
    getRoles: function() {
      return this.relation("belongRole");
    },
...
(function(root) {
   ...
   * フェッチが成功したかどうかにより、コールバック関数がsuccessかerrorか実行させる。
   ...
   * ものサンプルコードは <code>MyClass</code> クラスのオブジェクトと<code>myId</code>IDをフェッチするコードである。
   * フェッチが成功したかどうかにより、コールバック関数がsuccessかerrorか実行させる。
   * 
   * <pre>
   * var query = new NCMB.Query(MyClass);
  ...
  NCMB.Query.prototype = {
    ...
    /**
     * 正規表現にマッチする文字列を取得るすため、
     * 制限をクエリに追加する。
     * データーセットが大きい場合、実行速度に恐れがある。
     * @param {String} key マッチする文字列のキーである。
     * @param {RegExp} regex マッチする正規表現のパターン。
     * @return {NCMB.Query} チェーンにするクエリを返却する。
     */
...
(function(root) {
   ...
   * @class
   *
   * <p>NCMB.UserオブジェクトはNCMBクラウドに保存するユーザーのロカール表現である。
   * NCMB.Objectのサブクラスであり、NCMB.Objectと同じ関数を持っており、ユーザー関数をより多くメソッドを拡張する。
   * 例えば、ユーザー認証、サインアップ、独自性のバリデーションなど.
   </p>
   */
...
(function(root) {
   ...
   * @class
   *
   * <p>モデルのセットの標準コレクションを提供する。
   * コレクションの順番はソートおよび未ソート。
   * 詳しく確認は以下のリンクで確認ください。
...
(function(root) {
  ...
  NCMB.Installation = NCMB.Object.extend("_Installation");
  ...
  NCMB.Push = NCMB.Object.extend("Push",{
     ...
     *     <li>immediateDeliverFlag - 即自配信. </li>
     *     <li>target - ターゲット. </li>
     *     <li>searchCondition -  NCMB.Installation設定にマッチするクエリNCMB.Query.</li>
     *     <li>message - プッシュで送信するデーター</li>
     *     <li>deliveryExpirationDate -  プッシュを終了させるタイミング、デートオブジェクト. </li>
     *     <li>deliveryExpirationTime -  プッシュを終了させるタイミング、デートオブジェクト. </li>
...
(function(root) {
  ...
  /**
   * Facebookと統合するために利用するメソッドのセットを提供するクラス。
   * @namespace
   *  Facebookと統合するために利用するメソッドのセットを提供するクラス。
   */
  NCMB.FacebookUtils = {
...
(function(root) {
   ...
   * @class
   *
   * <p>ユーザーのために便利にするため、ビュー Backbone互換のViewを提供する。
   * このクラスを利用するため、jQueryかjqueryの$に互換ライブラリーを含めることが必要である。
...
(function(root) {
   ...
   * @class
   *
   * <p>Backbone.Router互換し、ユーザーが使いやすくするため提供する。
   * 詳しくは、以下のURKに参照すること。
...
(function(root) {
   ...
   * @class
   *   
   * <p>ユーザーのために便利にするため、ビュー Backbone互換のRouterを提供する。
   * このクラスを利用するため、jQueryかjqueryの$に互換ライブラリーを含めることが必要である。
   * 詳しくは<a href="http://documentcloud.github.com/backbone/#View">Backbone
